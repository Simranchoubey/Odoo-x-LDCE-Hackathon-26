...
async function main() {
  ...
  const demo = await prisma.user.upsert({
    ...
    create: {
      ...
      passwordHash,
      name: 'Demo Traveler',
      bio: 'Exploring the world one itinerary at a time.',
    },
  });

  const existingTrips = await prisma.trip.count({ where: { userId: demo.id } });
  if (existingTrips > 0) {
    console.log('Demo trips already seeded, skipping');
    return;
  }
  ...
  // [name, description, startDate, endDate, status, budget, coverImage, stops[]]
  // stop: [title, cityName|country, arrival, departure, budget, items[[activityName|null, titleOverride, date, time]]]
  const tripDefs = [
    ['Paris Getaway', 'A romantic 5-day escape to the City of Light, exploring museums, cafes, and iconic landmarks.', '2026-09-15', '2026-09-20', 'completed', 2400,
      [['Flight & Arrival', 'Paris|France', '2026-09-15', '2026-09-15', 800, [
        ['CDG Airport Transfer', null, '2026-09-15', '08:00'],
        ['Hotel Check-in - Le Marais', null, '2026-09-15', '14:00']]]]],
    ['Kyoto Retreat', 'A 4-day journey through ancient temples, bamboo forests, and traditional tea ceremonies.', '2026-10-11', '2026-10-14', 'upcoming', 1800,
      [['Arrival & Fushimi Inari', 'Kyoto|Japan', '2026-10-11', '2026-10-12', 400, [
    ...
    ['Bali Beach Escape', 'Eight days of beach, temples, rice terraces, and spiritual renewal in the Island of the Gods.', '2026-11-20', '2026-11-28', 'upcoming', 2000,
    ...
    ['NYC Winter Break', "A week-long urban adventure exploring Manhattan's neighborhoods, world-class museums, and Broadway shows.", '2026-08-01', '2026-08-07', 'ongoing', 3500,
      [['Midtown & Central Park', 'New York|USA', '2026-08-01', '2026-08-02', 600, [
        ['Central Park Bike Tour', null, '2026-08-01', '14:00'],
        ['Broadway Show Night', null, '2026-08-01', '20:00']]]]],
    ['Santorini Honeymoon', 'Seven magical days of whitewashed villas, volcanic black beaches, and spectacular sunsets.', '2026-06-10', '2026-06-17', 'completed', 5000,
      [['Oia & Sunset Views', 'Santorini|Greece', '2026-06-10', '2026-06-13', 2000, []]]],
    ['Morocco Souk Adventure', 'Five days of medina exploration, desert day trips, and authentic Moroccan cuisine.', '2026-12-05', '2026-12-10', 'planning', 1500, []],
  ];

  for (const [name, description, startDate, endDate, status, totalBudget, stops] of tripDefs) {
          ...
          create: (await Promise.all(stops.map(async ([title, cityKey, arrival, departure, budget, items], idx) => ({
            ...
            items: {
              create: await Promise.all(items.map(async ([activityName, titleOverride, date, time]) => {
                const a = activityName ? await act(activityName) : null;
                return {
  ...
  console.log('Seeded demo user + 6 trips');
}
